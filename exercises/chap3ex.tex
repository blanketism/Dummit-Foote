...
\begin{problems}
    ...
    \end{solalph}
    \item Prove that $\speciallinear_n(F) \nsub \gl_n(F)$ and describe the isomorphism type of the quotient group (cf. Exercise 9, Section 2.1).
    \begin{sol}
        We know that $\speciallinear_n(F) \leq \gl_n(F)$, so we just need to show that $A \speciallinear_n(F) A\inv = \speciallinear_n(F)$ for all $A \in \gl_n(F)$. To that end, note that for any $S \in \speciallinear_n(F)$ and $X \in \gl_n(F)$, we have $\det(XSX\inv) = \det(X)\det(S)\det(X\inv) = 1$, hence $X\speciallinear_n(F)X\inv \subseteq \speciallinear_n(F)$, and $\speciallinear_n(F) \nsub \gl_n(F)$.

        Recall that when a subgroup is normal, it is actually the kernel of some homomorphism. Observe that every element of $\speciallinear_n(F)$ has determinant 1, so if we consider the mapping $A \mapsto \det(A)$ for some $A \in \gl_n(F)$, then the kernel of this homomorphism is clearly $\speciallinear_n(F)$. We may then consider the mapping $\phi : \gl_n(F)/\speciallinear_n(F) \to F\unt$ given by $\phi(\bar X) = \det(X)$.

        We now show that this mapping is well defined. To that end, suppose $\bar A = \bar B$ for some $\bar A, \bar B \in \gl_n(F)/\speciallinear_n(F)$. Recall that the elements of $\bar A$ are of the form $AS$ for $A \in \gl_n(F)$ and some $S \in \speciallinear_n(F)$. Since $\bar A = \bar B$, then for $AS \in \bar A$ there exists $S' \in \speciallinear_n(F)$ such that $AS = BS_0$. Then
        \[\phi(\bar A) = \det(A) = \det(AS) = \det(BS') = \det(B) = \phi(\bar B)\]
        so that $\phi$ is well defined.
        
        To show that $\phi$ is injective, suppose $\phi(\bar A) = \phi(\bar B)$. Then $\det(A) = \det(B)$. Pick some $AS \in \bar A$, where $S \in \speciallinear_n(F)$. Observe that $B\inv AS \in \speciallinear_n(F)$, since $\det(B\inv AS) = \det(B\inv)\det(A)\det(S) = \det(A)\inv\det(A)\det(S) = 1$, hence $B\inv AS = S'$ for some $S' \in \speciallinear_n(F)$. Then $AS = BS'$, and $AS \in \bar B$ so that $\bar A \subseteq \bar B$. A similar argument shows that $\bar B \subseteq \bar A$ so that $\bar A = \bar B$, and $\phi$ is injective. Moreover, for some $f \in F\unt$, then $\det(fI_n) = f\det(I_n) = f$ so that $\phi$ is surjective. Lastly, for $\bar A, \bar B \in \speciallinear_n(F)$, then 
        \[\phi(\bar{AB}) = \det(AB) = \det(A)\det(B) = \phi(\bar A)\phi(\bar B)\]
        so that $\phi$ is a homomorphism. Then $\phi$ is a bijective homomorphism, and $\gl_n(F)/\speciallinear_n(F) \cong F\unt$. 
    \end{sol}
