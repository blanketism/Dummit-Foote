...
\begin{solenum}
    ...
    \item Since $Z(Q_8) = \gen{-1}$, then $N_{Q_8}(\gen{-1}) = Q_8$.
...
\begin{sol}
    Since $\abs v = \abs{uv} = 8$, then $\gen v \cong \gen{uv} \cong Z_8$.
...
\begin{sol}
	...
	Since it is not generated by one element, each of these elements are of order 2, and $v^4u = v^3uv^5 = \cdots = uv^{20} = uv^4$ so that it is Abelian, then $\gen{u, v^4} \cong V_4$.
\end{sol}
...
\begin{sol}
    ...
    \begin{align*}
        ...
        C_{QD_{16}}(\gen{\tau\sigma^2}) = C_{QD_{16}}(\gen{\tau\sigma^6}) & = \gen{\tau\sigma^2, \sigma^4} \qh
    \end{align*}
\end{sol}
