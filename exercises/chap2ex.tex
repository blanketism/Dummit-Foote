...
\begin{problems}
    ...
    \begin{sol}
        ...
        Since we know that $\speciallinear_2(\f_3)$ has order 24, we need to exhibit at least 13 distinct matrices using $A$ and $B$, since $\gen{A, B}$ divides 24. Since $I, A,$ and $B$ are 3 distinct matrices, we compute 10 more:
        \begin{align*}
        ...
        \end{align*}
        Hence $\gen{A, B} = 24$ so that $\gen{A, B} = \speciallinear_2(\f_3)$.
    \end{sol}
    ...
    \begin{sol}
        Recall that $Q_8$ and $S_4$ both have 6 elements of order 4. However, no two elements in $Q_8$ can generate the entirety of $\speciallinear_2(\f_3)$ since $\abs{Q_8} = 8$, while $S_4 = \gen{(1\ 2\ 3\ 4), (1\ 2\ 4\ 3)}$, hence they cannot be isomorphic.
    \end{sol}
