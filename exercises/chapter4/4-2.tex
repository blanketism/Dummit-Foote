...
\begin{exercise}{4.2.1}
	...
	\begin{subexercises}
		...
		\[
			a \mapsto \cyc{1 2}\cyc{3 4}, \quad b \mapsto \cyc{1 4}\cyc{2 3}, \quad c \mapsto \cyc{1 3}\cyc{2 4}.
		\]
...
\begin{solenum}
	...
	We also have $a \cdot b = c$ so that $\sigma_a(4) = 3$, and $\sigma_a(3) = 4$, and we have $a \mapsto \cyc{1 2}\cyc{3 4}$.
	The others are computed similarly. 
	\item Again, we have $a \cdot 1 = a$ so that $\sigma_a(1) = 4$.
	Similarly, $\sigma_a(4) = 1$.
	We also have $a \cdot b = c$ so that $\sigma_a(2) = 3$, and $\sigma_a(3) = 2$, and we have $a \mapsto \cyc{1 4}\cyc{2 3}$.
	For $b$, we have $b \mapsto \cyc{1 2}\cyc{3 4}$, and for $c$, we have $c \mapsto \cyc{1 3}\cyc{2 4}$.
	The image of $G$ in $S_4$ under this labelling is $\set{1, \cyc{1 4}\cyc{2 3}, \cyc{1 2}\cyc{3 4}, \cyc{1 3}\cyc{2 4}}$, which is the same subgroup as in part (a).
\end{solenum}

\begin{exercise}{4.2.2}
	List the elements of $S_3$ as $1, \cyc{1 2}, \cyc{2 3}, \cyc{1 3}, \cyc{1 2 3}, \cyc{1 3 2}$ and label these with the integers $1,2,3,4,5,6$ respectively.
...
\begin{sol}
	...
	\begin{align*}
		\cyc{1 2} \cdot 1 & = \cyc{1 2} && \sigma_{\cyc{1 2}}(1) = 2 \\
		\cyc{1 2} \cdot \cyc{1 2} & = 1 && \sigma_{\cyc{1 2}}(2) = 1 \\
		\cyc{1 2} \cdot \cyc{2 3} & = \cyc{1 2 3} && \sigma_{\cyc{1 2}}(4) = 5 \\
		\cyc{1 2} \cdot \cyc{1 3} & = \cyc{1 3 2} && \sigma_{\cyc{1 2}}(3) = 6 \\
		\cyc{1 2} \cdot \cyc{1 2 3} & = \cyc{2 3} && \sigma_{\cyc{1 2}}(5) = 3 \\
		\cyc{1 2} \cdot \cyc{1 3 2} & = \cyc{1 3} && \sigma_{\cyc{1 2}}(6) = 4
	\end{align*}
	Thus, we have $\cyc{1 2} \mapsto \cyc{1 2}\cyc{3 6 4 5}$.
	We calculate that $\cyc{2 3} \mapsto \cyc{1 3 5 2}\cyc{4 6}$.
	Since $\cyc{1 2}\cyc{2 3} = \cyc{1 2 3}$,
	\[
		\cyc{1 2 3} \mapsto \cyc{1 2}\cyc{3 6 4 5}\cyc{1 3 5 2}\cyc{4 6} = \cyc{1 5 6}\cyc{2 4 3}
	\]
	...
	\begin{align*}
		1 & \mapsto 1 \\
		\cyc{1 2} & \mapsto \cyc{1 2}\cyc{3 6 4 5} \\
		\cyc{2 3} & \mapsto \cyc{1 3 5 2}\cyc{4 6} \\
		\cyc{1 3} & \mapsto \cyc{1 4}\cyc{2 6}\cyc{3 5} \\
		\cyc{1 2 3} & \mapsto \cyc{1 5 6}\cyc{2 4 3} \\
		\cyc{1 3 2} & \mapsto \cyc{1 6 5}\cyc{2 3 4} \qedhere
	\end{align*}
\end{sol}
...
\begin{solenum}
	\item We calculate $\sigma_r = \cyc{1 2 3 4}\cyc{5 8 7 6}$ and $\sigma_s = \cyc{1 5}\cyc{2 6}\cyc{3 7}\cyc{4 8}$.
	...
	\begin{align*}
		1 & \mapsto 1 & s & \mapsto \cyc{1 5}\cyc{2 6}\cyc{3 7}\cyc{4 8} \\
		r & \mapsto \cyc{1 2 3 4}\cyc{5 8 7 6} & sr & \mapsto \cyc{1 6}\cyc{2 7}\cyc{3 8}\cyc{4 5} \\
		r^2 & \mapsto \cyc{1 3}\cyc{2 4}\cyc{5 7}\cyc{6 8} & sr^2 & \mapsto \cyc{1 7}\cyc{2 8}\cyc{3 5}\cyc{4 6} \\
		r^3 & \mapsto \cyc{1 4 3 2}\cyc{5 6 7 8} & sr^3 & \mapsto \cyc{1 8}\cyc{2 5}\cyc{3 6}\cyc{4 7}
	\end{align*}
	\item The images of $D_8$ under the left regular representation with respect to this new labelling are as follows:
	\begin{align*}
		1 & \mapsto 1 & s & \mapsto \cyc{1 2}\cyc{3 4}\cyc{5 6}\cyc{7 8} \\
		r & \mapsto \cyc{1 3 5 7}\cyc{2 8 6 4} & sr & \mapsto \cyc{1 4}\cyc{2 7}\cyc{3 6}\cyc{5 8} \\
		r^2 & \mapsto \cyc{1 5}\cyc{3 7}\cyc{2 6}\cyc{4 8} & sr^2 & \mapsto \cyc{1 6}\cyc{2 5}\cyc{3 8}\cyc{4 7} \\
		r^3 & \mapsto \cyc{1 7 5 3}\cyc{2 4 6 8} & sr^3 & \mapsto \cyc{1 8}\cyc{2 3}\cyc{4 5}\cyc{6 7}
	\end{align*}
...
\begin{sol}
	...
	We find that
	\begin{align*}
		i & \mapsto \sigma_i = \cyc{1 3 2 4}\cyc{5 7 6 8} \\
		j & \mapsto \sigma_j = \cyc{1 5 2 6}\cyc{3 8 4 7}
	\end{align*}
...
\begin{solenum}
	\item We have $\pi_H(r) = \cyc{1 2 3 4}$ and $\pi_H(s) = \cyc{2 4}$.
	We continue in this way to find that the images of the elements of $D_8$ under $\pi_H$ are:
	\begin{align*}
		\pi_H(1) & = 1 & \pi_H(s) & = \cyc{2 4} \\
		\pi_H(r) & = \cyc{1 2 3 4} & \pi_H(sr) & = \cyc{1 2}\cyc{3 4} \\
		\pi_H(r^2) & = \cyc{1 3}\cyc{2 4} & \pi_H(sr^2) & = \cyc{1 3} \\
		\pi_H(r^3) & = \cyc{1 4 3 2} & \pi_H(sr^3) & = \cyc{1 4}\cyc{2 3}
	\end{align*}
	so that the subgroup $\gen{\pi_H(r), \pi_H(s)} \leq S_4$ is isomorphic to $D_8$.
	Since the kernel of $\pi_H$ is trivial, the representation is faithful.
	\item The new labelling affords the permutations $\pi_H(r) = \cyc{1 3 2 4}$ and $\pi_H(s) = \cyc{3 4}$.
	We have the following images:
	\begin{align*}
		\pi_H(1) & = 1 & \pi_H(s) & = \cyc{3 4} \\
		\pi_H(r) & = \cyc{1 3 2 4} & \pi_H(sr) & = \cyc{1 3}\cyc{2 4} \\
		\pi_H(r^2) & = \cyc{1 2}\cyc{3 4} & \pi_H(sr^2) & = \cyc{1 2} \\
		\pi_H(r^3) & = \cyc{1 4 2 3} & \pi_H(sr^3) & = \cyc{1 4}\cyc{2 3}
	\end{align*}
	Moreover, the subgroup is different from that in part (a) since, for example, the image of $r$ in part (a) is $\cyc{1 2 3 4}$ while the image of $r$ in this part is $\cyc{1 3 2 4}$.
	\item Under $\pi_K$, we have $\pi_K(r) = \cyc{1 2 3 4}$ and $\pi_K(s) = \cyc{1 2}\cyc{3 4}$.
	With respect to this labeling, we have the subgroup $\wh K = \gen{\cyc{1 2 3 4}, \cyc{1 2}\cyc{3 4}}$.
	Let $\wh H$ be the subgroup obtained in part (a).
	Note that $\cyc{1 2 3 4}$ is contained in both $\wh H$ and $\wh K$.
	Moreover, we have following:
	\[
		\cyc{1 2 3 4}\cyc{2 4}\cyc{1 4 3 2} = \cyc{1 2}\cyc{3 4} \in \wh H \quad \text{and} \quad \cyc{1 2 3 4}^2\cyc{1 2}\cyc{3 4} = \cyc{2 4} \in \wh K
	\]
...
\begin{sol}
	We have $\pi_N(r) = \cyc{1 2}\cyc{3 4}$ and $\pi_N(s) = \cyc{1 3}\cyc{2 4}$.
	Continuing in this way, we find that the images of the elements of $D_8$ under $\pi_N$ are:
	\begin{align*}
		\pi_N(1) & = 1 & \pi_N(s) & = \cyc{1 3}\cyc{2 4} \\
		\pi_N(r) & = \cyc{1 2}\cyc{3 4} & \pi_N(sr) & = \cyc{1 4}\cyc{2 3} \\
		\pi_N(r^2) & = 1 & \pi_N(sr^2) & = \cyc{1 3}\cyc{2 4} \\
		\pi_N(r^3) & = \cyc{1 2}\cyc{3 4} & \pi_N(sr^3) & = \cyc{1 4}\cyc{2 3}
	\end{align*}
	Since $\ker(\pi_N)$ contains the non-trivial element $r^2$, the representation is not faithful.
	Moreover, we have $\pi_N(D_8) = \set{1, \cyc{1 2}\cyc{3 4}, \cyc{1 3}\cyc{2 4}, \cyc{1 4}\cyc{2 3}}$, which is isomorphic to the Klein 4-group.
\end{sol}
...
\begin{solenum}
	...
	For any $a \in A$, consider the orbit $\ocal_a$ of $a$ under this action.
	By Proposition 4.2, we have $|\ocal_a| = |Q_8 : (Q_8)_a|$, where $(Q_8)_a$ is the stabilizer of $a$ in $Q_8$.
	Since $|\ocal_a|$ divides $|A| \leq 7$, $|\ocal_a|$ must be 1, 2, 4, or 7.
	However, since $Q_8$ has no subgroup of index 7, $|\ocal_a|$ cannot be 7.
	If $|\ocal_a| = 4$, then $(Q_8)_a$ has order 2, but the only subgroup of order 2 in $Q_8$ is $\langle -1 \rangle$.
	If $|\ocal_a| = 2$, then $(Q_8)_a$ has order 4, and the only subgroups of order 4 in $Q_8$ are $\langle i \rangle$, $\langle j \rangle$, and $\langle k \rangle$, all of which contain $\langle -1 \rangle$.
	If $|\ocal_a| = 1$, then $(Q_8)_a = Q_8$, which also contains $\langle -1 \rangle$.
	Therefore, in all cases, the stabilizer $(Q_8)_a$ contains $\langle -1 \rangle$.
	Since this holds for all $a \in A$, $\langle -1 \rangle$ is contained in the kernel of the action.
...
\begin{sol}
	...
	Let $K = \ker(\phi) \subseteq H$, since $g \in K$ if and only if $gxH = xH$ for all $x \in G$.
	Taking $x = 1$, we have $gH = H$, so that $g \in H$.
	By the First Isomorphism Theorem, we have $G/K \cong \phi(G) \leq S_n$, so that $|G : K| = |G/K| = |\phi(G)|$ divides $|S_n| = n!$.
	Therefore, $|G : K| \leq n!$.
\end{sol}

\begin{exercise}{4.2.9}
	Prove that if $p$ is a prime and $G$ is a group of order $p^\alpha$ for some $\alpha \in \zbb^+$, then every subgroup of index $p$ is normal in $G$.
...
\begin{sol}
	...
	Since $|G| = p^\alpha$, by Lagrange's Theorem, the order of $\phi(G)$ divides $p^\alpha$.
	However, the only subgroups of $S_p$ whose order divides $p^\alpha$ are the trivial group and groups of order $p$.
	Since $\phi(G)$ acts transitively on the $p$ cosets of $H$, $\phi(G)$ cannot be trivial.
	...
	Since the image $\phi(G)$ has order $p$, by the First Isomorphism Theorem, we have $|G : \ker(\phi)| = p$.
	But since $\ker(\phi) \subseteq H$ and both have index $p$, $\ker(\phi) = H$.
...
\begin{sol}
	...
	Since $K$ is also Abelian as it is cyclic, $G = hk$ for $h \in H$ and $k \in K$, implying that $G$ is Abelian, a contradiction.
	Therefore, $H$ is not normal in $G$.

	To classify groups of order 6, let $G$ be a group of order 6.
	If $G$ is Abelian, then $G \cong \zbb_6 \cong \zbb_2 \times \zbb_3$.
	...
	Since $H$ is not normal in $G$, the action is non-trivial, so $\phi$ is injective.
	Therefore, $G \cong \phi(G) \leq S_3$.
	Since $|G| = 6 = |S_3|$, $\phi(G) = S_3$, and hence $G \cong S_3$.
\end{sol}
...
\begin{sol}
	...
	The orbit of any $g \in G$ under this action is $\ocal_g = \set{x^k g \mid k = 0, 1, \ldots, n-1}$.
	Since $\abs x = n$, $|\ocal_g| = n$ for all $g \in G$.
	By Proposition 4.2, we have $|\ocal_g| = |\langle x \rangle : (\langle x \rangle)_g|$, where $(\langle x \rangle)_g$ is the stabilizer of $g$ in $\langle x \rangle$.
	Since $|\ocal_g| = n$, $(\langle x \rangle)_g$ is trivial for all $g \in G$.
...
\begin{sol}
	...
	Since $\pi(G)$ contains an odd permutation, $\ep \circ \pi$ is surjective.
...
\begin{sol}
	...
	Since $|G| = 2k$ with $k$ odd, use $m = k$ and $n = 2$ in \exref{4.2.11} along with even $\abs x$ to conclude that $\pi(x)$ is an odd permutation.
