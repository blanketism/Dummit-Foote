...
\begin{solenum}
    ...
    \]
    Since $G_I$ and $\prod_{i \in I} G_i$ are both groups under componentwise multiplication and $\phi$ preserves multiplication componentwise, $\phi$ is a homomorphism.
...
\begin{solenum}[spsol]
    ...
    Let $\mb x = (x, 1)Z_D$ be the coset of $Z_D$ in $Z_4 \ast D_8$, and denote all other elements of $Z_4 \ast D_8$ and $Z_4 \ast Q_8$ similarly.

    A preliminary observation is that we need an element in $Z_4 \ast Q_8$ that corresponds to $\mb s \in Z_4 \ast D_8$.
    We claim that the element $\mb z = \mb j \mb y\inv$ has order 2 and conjugates $\mb i$ to $\mb i\inv$.
    Note that $|\mb z| \neq 1$, for otherwise $\mb j = \mb y$. This would imply $\mb j \in \pi_{Z_4}(y) = \gen{\mb y}$, but $\mb j$ does not commute with $\mb i$, so $\mb j \notin \gen{\mb y}$. Then
    \[
        \mb z^2 = \mb j \mb y\inv \mb j \mb y\inv = \mb j^2 \mb y^{-2} = \mb 1,
    \]
    so $\mb z$ has order 2.
    Moreover, we have
    \[
        \mb z \mb i \mb z\inv = \mb j \mb y\inv \mb i \mb y \mb j\inv = \mb j \mb i \mb j\inv = \mb i\inv.
    \]
    Then $\mb z$ has the same properties as $\mb s$, so we may define a map $\phi : Z_4 \ast D_8 \to Z_4 \ast Q_8$ by
    \[
        \phi(\mb x^\alpha \mb r^\beta \mb s^\gamma) = \mb y^\alpha \mb i^\beta \mb z^\gamma.
    \]
    ...
    In particular, we have in $Z_4 \ast D_8$ that $\mb r^4 = \mb s^2 = \mb 1$, $\mb x^2 = \mb r^2$, and $\mb s\mb r\mb s = \mb r\inv$.
    Then in $Z_4 \ast Q_8$, we have
    \[
        \phi(\mb r^4) = \mb i^4 = \mb 1, \quad \phi(\mb s^2) = \mb z^2 = \mb 1, \quad \phi(\mb x^2) = \mb y^2 = \mb i^2 = \phi(\mb r^2), \quad \text{and} \quad \phi(\mb s\mb r\mb s) = \mb z\mb i\mb z = \mb i\inv.
    \]
    Additionally, $\mb y$ is central in $Z_4 \ast Q_8$, which follows from the fact that $Z_4$ is central in $Z_4 \ast D_8$ and $\phi$ preserves the relations.
    Thus, $\phi$ is a homomorphism.

    To see that $\phi$ is surjective, note that any element of $Z_4 \ast Q_8$ can be written as $\mb y^\alpha \mb i^\beta \mb z^\gamma$, which is the image of $\mb x^\alpha \mb r^\beta \mb s^\gamma$ under $\phi$.
    Since $Z_4$ and $Q_8$ are generated by $\mb y$ and $\mb i, \mb j$ respectively, putting $\mb j = \mb z \mb y$ shows that $\mb y, \mb i, \mb z$ generate $Z_4 \ast Q_8$.
...
\begin{sol}
    To construct the presentations, we note that $\mb x$ should satisfy the relations of $Z_4$, $\mb r$ and $\mb s$ should satisfy the relations of $D_8$, and $\mb i$ and $\mb z$ should satisfy the relations of $Q_8$.
	Moreover, we have the relations $\mb x^2 = \mb r^2$ in $Z_4 \ast D_8$, since $\mb x^2\mb r^{-2} = \mb 1$.
	Lastly, $\mb x$ is central in that $\mb x \mb r = \mb r\mb x$ and $\mb x\mb s = \mb s\mb x$ in $Z_4 \ast D_8$.
	Then a presentation for $Z_4 \ast D_8$ is
    \[
    	\gen{\mb x, \mb r, \mb s \mid \mb r^4 = \mb s^2 = \mb 1, \mb x^2 = \mb r^2, \mb s\mb r\mb s = \mb r\inv, \mb x\mb r = \mb r\mb x, \mb x\mb s = \mb s\mb x}.
    \]
    To find a presentation for $Z_4 \ast Q_8$, we may use the isomorphism $\phi$ from the preceding exercise to rewrite the presentation for $Z_4 \ast D_8$ in terms of $\mb y, \mb i, \mb z$.
	In particular, we have $\mb y^2 = \mb i^2\mb z^2$, $\mb z^2 = \mb 1$, and $\mb y\mb i = \mb i\mb y\mb z$.
	Thus, a presentation for $Z_4 \ast Q_8$ is
    \[
    	\gen{\mb y, \mb i, \mb z \mid \mb i^4 = \mb z^2 = \mb 1, \mb y^2 = \mb i^2, \mb z\mb i \mb z = \mb i\inv, \mb y\mb i = \mb i\mb y, \mb y\mb z = \mb z\mb y}. \qedhere
    \]
\end{sol}
