...
\begin{soliff}
    \rightimp Let $g = (g_1, g_2, \ldots, g_t) \in G$ have order $m$.
    ...
    \leftimp If $m \mid n_1$, then there exists an element of order $m$ in the cyclic subgroup of order $n_1$.
...
\begin{sol}
	...
	Elements of order 2 in $A$ consist of elements of order 2 in the 2-power part of $A$ and elements of order 1 in the other parts of $A$.
    The 2-power map sends $(a, b, c) \in A$ to $(2a, 2b, 2c)$, so the kernel of the 2-power map is $\set{0, 2} \times \set{0, 2} \times \set{0, 2}$, which has order $2^3 = 8$.
    The kernel contains 7 non-identity elements, so there are 7 elements of order 2 in $A$.

    To see the number of subgroups of index 2 in $A$, note that index 2 subgroups of $A$ are kernels of surjective homomorphisms from $A$ to $Z_2$.
    Since $A$ is Abelian, any homomorphism from $A$ to $Z_2$ is determined by the images of the generators of $A$.
    Note that there are exactly 2 homomorphisms from $Z_4$ to $Z_2$, which is $x \mapsto 1$ and $x \mapsto y$ for $Z_2 = \gen y$ and $Z_4 = \gen x$.
    Thus, there are $2^3 = 8$ homomorphisms from the 2-power part of $A$ to $Z_2$.
    Since one of these homomorphisms is the trivial homomorphism, there are 7 surjective homomorphisms from the 2-power part of $A$ to $Z_2$, hence there are 7 subgroups of index 2 in $A$.
\end{sol}
...
\begin{solenum}[spsol]
	...
	Since $s$ is odd, $k \equiv 0 \bmod 8$.
	Then $kt \equiv 0 \bmod 4$, hence $\ell v \equiv 0 \bmod 4$.
	Since $v$ is odd, $\ell \equiv 0 \bmod 4$, so we have trivial intersection.
        \item $u = 4$ and $v$ is odd: We have $ks \equiv 4\ell \bmod 8$ and $kt \equiv \ell v \bmod 4$.
	Since $s$ is odd, $k \equiv 4\ell s\inv \bmod 8$.
	...
	Since $s$ is odd, $k \equiv 2 \ell s\inv \bmod 8$.
	...
	Since $v$ is odd, $2s\inv - v$ is odd, so $\ell \equiv 0 \bmod 4$, so we have trivial intersection.
    \end{enumerate}
