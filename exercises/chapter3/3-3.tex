...
\begin{sol}
	...
	Since $F$ is a finite field of order $q$, $\abs{F\unt} = q - 1$.
...
\begin{sol}[spsol]
	...
	Since $1 \in A$ for any $A \in \acal$, $1N = N \in \bar A$ so that $\bar 1 \in \bar A$, hence $\bar A$ is nonempty.
	Let $\bar a, \bar b \in \bar A$, where $a, b \in A$.
	Since $A \leq G$, $ab\inv \in A$, hence
    \[
	...
	Since $\pi$ is a homomorphism, $\pi\inv(\bar A) \leq G$.
	Moreover, since $\bar 1 \in \bar A$, $N = \pi\inv(\bar 1) \subseteq \pi\inv(\bar A)$ so that $\pi\inv(\bar A) \in \acal$.
	Note that
    \[
    ...
    \begin{enumerate}
        ...
        Since $A \leq B$, $a \in B$ so that $\bar a \in \bar B$, hence $\bar A \leq \bar B$.
        
        \leftimp Suppose $\bar A \leq \bar B$ for $A, B \in \acal$.
        ...
        \]
        Since $A \nsub G$, $gag\inv \in A$, hence $\bar{gag\inv} \in \bar A$.
	    It follows that $(\bar g)(\bar a)(\bar g)\inv \in \bar A$, hence $\bar A \nsub \bar G$.
        
        \leftimp Suppose $\bar A \nsub \bar G$ for some $A \in \acal$.
        Let $g \in G$ and $a \in A$.
        Since $\bar{gag\inv} = (\bar g)(\bar a)(\bar g)\inv \in \bar A$, $gag\inv N \in A/N$, hence $gag\inv \in A$.
        It follows that $A \nsub G$. \qedhere
    \end{enumerate}
\end{sol}
...
\begin{sol}
    Since $H \nsub G$ and $K \leq G$, $HK \leq G$ by Corollary 3.15.
	...
	It follows that $K \leq H$. \qedhere
    \end{enumerate}
\end{sol}
...
\begin{sol}
    ...
    \[
    	(A \times B)/(C \times D) \cong (A/C) \times (B/D). \qedhere
    \]
\end{sol}
...
\begin{sol}
    ...
    \[
    	G/(M \cap N) \cong (G/M) \times (G/N). \qedhere
    \]
\end{sol}
...
\begin{sol}
	...
	Suppose $y \in G$.
    Then $y$ is a $p^n$-th root of unity for some $n \in \zp$, so that $y = e^{2\pi i k/p^n}$ for some $k \in \zbb$.
	Let $z = e^{2\pi i k/p^{n+1}}$, so that $z^p = e^{2\pi i k/p^n} = y$. Then $z \in G$, hence $\phi$ is surjective.

    Additionally, we have $\ker\phi = \set{z \in G \mid z^p = 1} = \set{e^{2\pi i k/p} \mid k = 0, 1, \ldots, p - 1}$, which is a nontrivial subgroup of $G$.
    By the First Isomorphism Theorem, we have $G/\ker\phi \cong \phi(G) = G$, hence $G$ is isomorphic to a proper quotient of itself.
\end{sol}
...
\begin{sol}[spsol]
    Since $P \cap N \leq P$, $|P \cap N|$ divides $|P| = p^a$.
	Similarly, since $P \cap N \leq N$, $|P \cap N|$ divides $|N| = p^b n$.
	...
	Since $PN/N \leq G/N$, $|PN/N|$ divides $|G/N|$.
	It follows that $p^{a - c}$ divides $|G|/|N| = (p^a m)/(p^b n) = p^{a - b}(m/n)$.
	Since $p$ does not divide $m/n$, $p^{a - c}$ divides $p^{a - b}$, hence $c \geq b$.
