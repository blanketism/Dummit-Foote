...
\begin{sol}
	...
	Let $z = e^{2\pi i k/p^{n+1}}$, so that $z^p = e^{2\pi i k/p^n} = y$. Then because $z \in G$, $\phi$ is surjective.

    Now $\ker\phi = \set{z \in G \mid z^p = 1} = \set{e^{2\pi i k/p} \mid k = 0, 1, \ldots, p - 1}$, which is a nontrivial subgroup of $G$.
