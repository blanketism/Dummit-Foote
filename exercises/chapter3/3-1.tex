...
\begin{sol}
    Since $e^{2\pi ir} = 1$ if and only if $r$ is an integer, then $\ker(\phi) = \zbb$.
...
\begin{solenum}
	...
	Moreover, $1/k + \zbb \in \qbb/\zbb$ has order $k$, and since $k$ can be made arbitrarily large, then there are elements of arbitrarily large order.
	...
	Since $\tor(\rbb/\zbb) = \qbb/\zbb$, then $\qbb/\zbb$ is isomorphic to the set of roots of unity.
\end{solenum}
...
\begin{sol}
	...
	Then $X\splin_n(F)X\inv \subseteq \splin_n(F)$ for every $X \in \gl_n(F)$, and since conjugation is an isomorphism, then $X\splin_n(F)X\inv = \splin_n(F)$.
	Hence, $\splin_n(F) \nsub \gl_n(F)$.
