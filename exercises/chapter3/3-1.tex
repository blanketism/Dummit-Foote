...
\begin{sol}
	...
	Since $E \leq H$, $\phi(xy\inv) = \phi(x)\phi(y)\inv \in E$, hence $xy\inv \in \phi\inv(E)$.
	Therefore, $\phi\inv(E) \leq G$.

    Suppose $E \nsub H$.
	Then $heh\inv \in E$ for all $h \in H$ and $e \in E$.
...
\begin{exercise}{3.1.4}
    Prove that in the quotient group $G/N$, $(gN)^{\alpha} = g^{\alpha}N$ for all $\alpha \in \zbb$.
\end{exercise}

\begin{sol}
    Inducting on $\alpha$, the base case $\alpha = 0$ is clear since $(gN)^0 = N = g^0N$.
	Suppose the statement holds for some $\alpha \in \zbb$.
	Then
    \[
...
\begin{exercise}{3.1.6}
    Define $\phi : \rbb^{\times} \to \set{\pm 1}$ by letting $\phi(x)$ be $x$ divided by the absolute value of $x$.
...
\begin{sol}
	...
	Then
    \[
    	\phi(xy) = \frac{xy}{\abs{xy}} = \frac{x}{\abs x} \cdot \frac{y}{\abs y} = \phi(x)\phi(y) \qedhere
    \]
\end{sol}

\begin{exercise}{3.1.7}
    Define $\pi : \rbb^2 \to \rbb$ by $\pi((x,y)) = x + y$.
...
\begin{sol}
	...
	Moreover, $\pi$ is surjective since $\pi((a, 0)) = a$ for all $a \in \rbb$.
	We have $\ker\pi$ as the line $y = -x$, and $\pi\inv(a)$ as $y = a - x$, which is $\ker\pi$ translated up by $a$.
\end{sol}

\begin{exercise}{3.1.8}
    Let $\phi : \rbb^{\times} \to \rbb^{\times}$ be the map sending $x$ to the absolute value of $x$.
...
\begin{sol}
	...
	Note that $\abs x > 0$ for any $x \in \rbb\unt$, so $\im(\phi) = \rbb^+$. $\ker(\phi) = \set{-1, 1}$, and the fiber of $\phi$ over some $a \in \rbb^+$ is simply $\set{-a, a}$.
\end{sol}

\begin{exercise}{3.1.9}
    Define $\phi : \cbb^{\times} \to \rbb^{\times}$ by $\phi(a+bi) = a^2 + b^2$.
	Prove that $\phi$ is a homomorphism and find the image of $\phi$.
	Describe the kernel and the fibers of $\phi$ geometrically (as subsets of the plane).
\end{exercise}

\begin{sol}
    Let $a + bi, c + di \in \cbb\unt$.
	Then
    \begin{align*}
    ...
    \end{align*}
    so that $\phi$ is a homomorphism.
	Observe that $a^2 + b^2 > 0$ for any $a + bi \in \cbb\unt$, so that $\im(\phi) = \rbb^+$. $\ker\phi$ is the unit circle in the complex plane, and the fiber of $\phi$ over some $r \in \rbb^+$ is the circle of radius $\sqrt r$ centered at the origin.
\end{sol}
...
\begin{sol}
	...
	Then $8 \mid (a - b)$, so that $a - b = 8k$ for some $k \in \zbb$.
...
\begin{sol}
    Since $e^{2\pi ir} = 1$ if and only if $r$ is an integer, $\ker(\phi) = \zbb$.
    ...
    \begin{align*}
        \phi\inv(-1) & = \frac 12 + \zbb = \set*{\left.\frac 12 + n~\right|~n \in \zbb} \\
        \phi\inv(i) & = \frac 14 + \zbb = \set*{\left.\frac 14 + n~\right|~n \in \zbb} \\
        \phi\inv(e^{4\pi i/3}) & = \frac 23 + \zbb = \set*{\left.\frac 23 + n~\right|~n \in \zbb} \qedhere
    \end{align*}
\end{sol}
...
\begin{sol}
    The kernel of $\phi$ is $\frac12\zbb$, or
    \begin{center}
        \begin{tikzpicture}
            \draw[<->] (-4.5, 0) -- (4.5, 0);
    ...
    \begin{align*}
        \phi\inv(-1) & = \frac14 + \frac12 \zbb = \set*{\left.\frac14 + \frac n2~\right|~n \in \zbb} \\
        \phi\inv(i) & = \frac18 + \frac12 \zbb = \set*{\left.\frac18 + \frac n2~\right|~n \in \zbb} \\
        \phi\inv(e^{4\pi i/3}) & = \frac13 + \frac12 \zbb = \set*{\left.\frac13 + \frac n2~\right|~n \in \zbb} \qedhere
    \end{align*}
\end{sol}

\begin{spex}{3.1.14}
    Consider the additive quotient group $\qbb/\zbb$.
    \begin{subexercises}
        \item Show that every coset of $\zbb$ in $\qbb$ contains exactly one representative $q \in \qbb$ in the range $0 \le q < 1$.
        \item Show that every element of $\qbb/\zbb$ has finite order but that there are elements of arbitrarily large order.
        \item Show that $\qbb/\zbb$ is the torsion subgroup of $\rbb/\zbb$ (cf. \exref{2.1.6}).
        \item Prove that $\qbb/\zbb$ is isomorphic to the multiplicative group of roots of unity in $\cbb^\times$.
    \end{subexercises}
\end{spex}

\begin{solenum}[spsol]
    \item Suppose $a/b \in \qbb$ with $(a, b) = 1$.
	By the Division Algorithm, there exist unique integers $q, r$ such that $a = bq + r$ for $0 \leq r < b$, or equivalently, $a/b = q + r/b$.
	Then $a/b + \zbb = r/b + \zbb$.
	If there exists some other representative $s/t$ in $0 \leq s/t < 1$ such that $a/b + \zbb = s/t + \zbb$, then $r/b + \zbb = s/t + \zbb$, or that $r/b - s/t \in \zbb$.
	Then there exists some integer $k$ such that $rt - sb = kbt$.
	Since $0 \leq r < b$ and $0 \leq s < t$, $0 \leq rt < bt$ and $0 \leq sb < bt$.
	Combining these two inequalities, we get $-bt < rt - sb < bt$, so that $-1 < k < 1$.
	Since $k$ is an integer, $k = 0$, and $rt - sb = 0$, or $r/b = s/t$.
	Hence, the representative in $0 \leq q < 1$ is unique.
    \item Let $a/b \in \qbb$.
	Then $b(a/b + \zbb) = ab + \zbb = \zbb$, so that $a/b + \zbb$ has finite order.
	Moreover, $1/k + \zbb \in \qbb/\zbb$ has order $k$, and since $k$ can be made arbitrarily large, there are elements of arbitrarily large order.
    \item By the previous exercise, $\qbb/\zbb \subseteq \tor(\rbb/\zbb)$.
	Let $x + \zbb \in \tor(\rbb/\zbb)$.
	Then $|x + \zbb|$ is finite, so there exists some $n \in \zbb$ such that $n(x + \zbb) = nx + \zbb = \zbb$.
	Then $nx \in \zbb$, so that $x = m/n$ for some $m \in \zbb$.
	Hence, $x + \zbb \in \qbb/\zbb$, and $\tor(\rbb/\zbb) \subseteq \qbb/\zbb$.
	Therefore, $\tor(\rbb/\zbb) = \qbb/\zbb$.
    \item By \exref{3.1.12}, we have $\rbb/\zbb \cong S^1$.
	By definition, $\tor(S^1)$ consists of $z \in \cbb\unt$ such that $z^n = 1$ for some $n \in \zp$.
	This is precisely the set of roots of unity.
	Since $\tor(\rbb/\zbb) = \qbb/\zbb$, $\qbb/\zbb$ is isomorphic to the set of roots of unity.
\end{solenum}

\begin{exercise}{3.1.15}
    Prove that a quotient of a divisible Abelian group by any proper subgroup is also divisible.
	Deduce that $\qbb/\zbb$ is divisible (cf. \exref{2.4.19}).
\end{exercise}
...
\begin{sol}
	...
	Observe that $\qbb$ is divisible, since for any $q \in \qbb$ and $n \in \zp$, there exists $p \in \qbb$ such that $np = q$, namely $p = q/n$.
	Moreover, $\zbb$ is a proper subgroup of $\qbb$, so by the first part, $\qbb/\zbb$ is divisible.
\end{sol}
...
\begin{sol}[spsol]
	...
	Since $G = \gen S$, for any $g \in G$, we have
    \[
    	g = s_1^{\alpha_1} s_2^{\alpha_2} \cdots s_k^{\alpha_k}, \quad s_i \in S, \alpha_i \in \zbb
    \]
	...
	Since $\bar s \in \bar G$ for any $s \in S$, $\gen{\bar S} \subseteq \bar G$.
...
\begin{solenum}
	...
	Since $\bar r^2$ commutes with both $\bar r$ and $\bar s$, $\bar r^2 \in Z(\bar G)$.
...
\begin{solenum}
    ...
    \[
        \begin{array}{c|c|c|c|c|c|c|c|c}
            ...
            \hline
            \abs{\bar x} & 1 & 4 & 2 & 4 & 2 & 4 & 2 & 4
        \end{array}
    \]
    \item $\bar{vu} = \bar{uv^5} = \bar{uv},\; \bar{uv^{-2}u} = \bar{v^2},\; \bar{u\inv v\inv uv} = \bar{v^2}$.
...
\begin{spex}{3.1.20}
    Let $G = \zbb/24\zbb$ and let $\wt{G} = G/\langle \bar{12} \rangle$, where for each integer $a$ we simplify notation by writing $\wt{\bar a}$ as $\wt{a}$.
    \begin{subexercises}
        \item Show that $\wt{G} = \set{ \wt{0}, \wt{1}, \ldots, \wt{11} }$.
        \item Find the order of each element of $\wt{G}$.
        \item Prove that $\wt{G} \cong \zbb/12\zbb$ (thus $(\zbb/24\zbb)/(12\zbb/24\zbb) \cong \zbb/12\zbb$, just as if we inverted and canceled the $24\zbb$'s).
    \end{subexercises}
\end{spex}
...
\begin{exercise}{3.1.21}
    ...
    \begin{subexercises}
        ...
        \item Find the order of each element of $\bar G$ exhibited in (b).
...
\begin{solenum}
	...
	Since $\gen{\bar{xy}} \cap \gen{\bar x}$ is trivial, $\bar{xy}^{s - u} = \bar x^{v - t}$ imply that both quantities must be one.
...
\begin{solenum}
	...
	Since $H \nsub G$ and $K \nsub G$, $gxg\inv \in H$ and $gxg\inv \in K$, hence $gxg\inv \in H \cap K$.
...
\begin{sol}
    ...
    \]
    Since $N_i \nsub G$, $gn_ig\inv \in N_i$ for each $1 \leq i \leq k$.
	Then
    \[
...
\begin{sol}
	...
	Since $N \nsub G$, $hxh\inv \in N$.
	Since $H \leq G$, $hxh\inv \in H$ so that $hxh\inv \in N \cap H$.
	Then $N \cap H \nsub H$.
\end{sol}
...
\begin{exercise}{3.1.25}
    \begin{subexercises}
        ...
        \item Let $G = \gl_2(\qbb)$, let $N$ be the subgroup of upper triangular matrices with integer entries and $1$'s on the diagonal, and let $g$ be the diagonal matrix with entries $2,1$.
...
\begin{solenum}
    ...
    \leftimp Suppose $gNg\inv \subseteq N$ for every $g \in G$, and let $n \in N$.
...
\begin{exercise}{3.1.26} 
    ...
    \begin{subexercises}
        ...
        \item Deduce that if $N$ is the cyclic group $\langle x \rangle$, then $N$ is normal in $G$ if and only if for each $g \in G$, $gxg\inv  = x^k$ for some $k \in \zbb$.
...
\begin{solenum}
    ...
    Since
    \[
        gng\inv = g(s_1s_2 \cdots s_k)g\inv = (gs_1g\inv)(gs_2g\inv) \cdots (gs_kg\inv)
    \]
    for every $g \in G$ and $gSg\inv \subseteq N$, it follows that $gng\inv \in N$.
    Hence, $gNg\inv \subseteq N$ for every $g \in G$, so that $N \nsub G$.
    \item \rightimp Immediate from the definition of a normal subgroup.

    \leftimp Let $S = \gen x$, and use part (c).
...
\begin{soliff}
    \rightimp Suppose $gNg\inv \subseteq N$.
...
\begin{soliff}
    ...
    \leftimp If $S$ is empty, then $N$ is trivial, hence is normal.
	Otherwise, suppose $S$ is nonempty, and let $n \in N$.
	Since $N = \gen S$,
    \[
    	n = s_1s_2 \cdots s_k, \quad s_i \in S \text{ for all } i = 1, 2, \ldots, k.
...
\begin{soliff}
    ...
    \leftimp Let $S$ and $T$ be nonempty subsets of $G$, and let $g \in G$.
	By assumption, $g \in \gen T$ implies that
    \[
...
\begin{soliff}
    \rightimp Suppose $gN = Ng$, and let $n \in N$.
    Then there exists $n' \in N$ such that $gn = n'g$.
    Then $gng\inv = n' \in N$, hence $gNg\inv \subseteq N$.
    Similarly, there exists $n'' \in N$ such that $ng = gn''$, so that $n = gn''g\inv \in gNg\inv$, hence $N \subseteq gNg\inv$.
    Then $gNg\inv = N$, so that $g \in N_G(N)$.

    \leftimp Suppose $g \in N_G(N)$.
	Let $n \in N$.
...
\begin{sol}
	...
	Moreover, since every subgroup such that $N$ is normal in is a subgroup of $N_G(N)$, $N_G(N)$ is the largest subgroup in which $N$ is normal.
\end{sol}
...
\begin{sol}
	...
	Since $j\gen i (-j) = \gen i$, $j \in N_{Q_8}(\gen i)$ so that $N_{Q_8}(\gen i) = Q_8$.
...
\begin{sol}
    ...
    \begin{align*}
        r\gen{s, r^2}r\inv & = \set{1, sr^2, r^2, s} = \gen{s, r^2} \\
        s\gen r s\inv & = \set{1, r^3, r^2, r} = \gen r \\
        r\gen{rs, r^2}r\inv & = \set{1, sr, r^2, sr^3} = \gen{rs, r^2}
    \end{align*}
    then the normalizers of each subgroup contain $r$ and $s$, hence $N_{D_8}(\gen{s, r^2}) = N_{D_8}(\gen r) = N_{D_8}(\gen{rs, r^2}) = D_8$.
	Moreover, each of these maximal subgroups are of order 4, which means their corresponding quotient groups will have order 2, hence $D_8/\gen{s, r^2} \cong D_8/\gen r \cong D_8/\gen{rs, r^2} \cong Z_2$.

    Now we examine $\gen{r^2} = Z(D_8)$, which is normal.
...
\begin{solenum}
	...
	Since $r, s \in N_{D_{2n}}(\gen{r^k})$, $\gen{r^k} \nsub D_{2n}$.
    \item Consider $D_{2n}/\gen{r^k}$.
	Since $k \mid n$, the order of $\gen{r^k}$ is $n/k$, and the number of cosets in the quotient group is $2n/(n/k) = 2k$.
...
\begin{spex}{3.1.35} 
    Prove that $\splin_n(F) \nsub \gl_n(F)$ and describe the isomorphism type of the quotient group (cf. \exref{2.1.9}).
\end{spex}

\begin{sol}[spsol]
    Since $\splin_n(F) \leq \gl_n(F)$ by \exref{2.1.9}, it remains to be shown that $\splin_n(F)$ is normal in $\gl_n(F)$.
	Let $S \in \splin_n(F)$ and $X \in \gl_n(F)$.
	Then
    \[
    	\det(XSX\inv) = \det(X)\det(S)\det(X\inv) = 1
    \]
    so that $XSX\inv \in \splin_n(F)$.
	Then $X\splin_n(F)X\inv \subseteq \splin_n(F)$ for every $X \in \gl_n(F)$, and since conjugation is an isomorphism, $X\splin_n(F)X\inv = \splin_n(F)$.
	Hence, $\splin_n(F) \nsub \gl_n(F)$.

    Observe that a normal subgroup corresponds to the kernel of some homomorphism.
	It is clear that the mapping $\det : \gl_n(F) \to F\unt$ is a homomorphism, and the elements that map to the identity in $F\unt$ are precisely those matrices with determinant 1, i.e. $\splin_n(F)$.
	Then we may consider the mapping $\phi : \gl_n(F)/\splin_n(F) \to F\unt$ given by $\phi(\bar X) = \det(X)$.

    We now show that $\phi$ is well-defined.
	Suppose $\bar A = \bar B$ for some $\bar A, \bar B \in \gl_n(F)/\splin_n(F)$.
	Then for some $AS \in \bar A$ for $S \in \splin_n(F)$, there exists $S' \in \splin_n(F)$ such that $AS = BS'$.
	Then
    \[
    ...
    Suppose $\phi(\bar A) = \phi(\bar B)$ for $\bar A, \bar B \in \gl_n(F)/\splin_n(F)$.
	Then $\det(A) = \det(B)$.
	Let $AS \in \bar A$ for some $S \in \splin_n(F)$.
	Note that $B\inv$ exists since $B \in \gl_n(F)$.
	Then
    \[
    	\det(B\inv AS) = \det(B\inv)\det(A)\det(S) = \det(A)\inv\det(A)\det(S) = 1
    \]
    so that $B\inv AS \in \splin_n(F)$, hence $B\inv AS = S'$ for some $S' \in \splin_n(F)$.
	...
	Lastly, for $\bar A, \bar B \in \splin_n(F)$, then
    \[
    	\phi(\bar{AB}) = \det(AB) = \det(A)\det(B) = \phi(\bar A)\phi(\bar B)
    \]
    so that $\phi$ is a homomorphism.
	Then $\phi$ is a bijective homomorphism, and $\gl_n(F)/\splin_n(F) \cong F\unt$.
\end{sol}

\begin{exercise}{3.1.36} 
    Prove that if $G/Z(G)$ is cyclic then $G$ is Abelian. [If $G/Z(G)$ is cyclic with generator $xZ(G)$, show that every element of $G$ can be written in the form $x^a z$ for some integer $a \in \zbb$ and some element $z \in Z(G)$.]
\end{exercise}

\begin{sol}
    Suppose $G/Z(G)$ is cyclic with generator $xZ(G)$.
	Let $g, h \in G$.
	Then there exist integers $a, b \in \zbb$ and elements $z_1, z_2 \in Z(G)$ such that
    \[
...
\begin{sol}
    Let $\alpha, \beta \in S_3$, where $\alpha = 1$ and $\beta = \cyc{1 2 3}$ so that $\alpha\inv = \alpha$ and $\beta\inv \neq \beta$.
	For $\gamma = \cyc{1 2} \in S_3$, consider $\alpha\gamma\alpha\inv$ and $\beta\gamma\beta\inv$.
	Observe that $\alpha\gamma\alpha\inv = \gamma$, while $\beta\gamma\beta\inv = \cyc{1 2 3}\cyc{1 2}\cyc{3 2 1} = \cyc{2 3} \neq \cyc{1 2} = \gamma$.
	Then $(\alpha, \beta)(\gamma, \gamma)(\alpha\inv, \beta\inv) = (\gamma, \cyc{2 3})$, hence $D$ is not a normal subgroup of $S_3 \times S_3$.
\end{sol}
...
\begin{soliff}[spsol]
    ...
    \leftimp Suppose $x\inv y\inv xy\in N$.
...
\begin{sol}
	...
	Since $H \nsub G$, $y\inv xy \in H$, hence $[x, y] \in H$.
	Since $K \nsub G$, $x\inv y\inv x\in K$, hence $[x, y] \in K$.
	Then $[x, y] \in H \cap K = 1$ so that $[x, y] = 1$.
	It follows that $x\inv y\inv xy = 1$, or $xy = yx$.
\end{sol}

\begin{spex}{3.1.43}
    Assume $\pcal = \set{A_i \mid i \in I}$ is any partition of $G$ with the property that $\pcal$ is a group under the ``quotient operation'' defined as follows: to compute the product of $A_i$ with $A_j$ take any element $a_i$ of $A_i$ and any element $a_j$ of $A_j$ and let $A_iA_j$ be the element of $\pcal$ containing $a_ia_j$ (this operation is assumed to be well-defined).
	Prove that the element of $\pcal$ that contains the identity of $G$ is a normal subgroup of $G$ and the elements of $\pcal$ are the cosets of this subgroup (so $\pcal$ is just a quotient group of $G$ in the usual sense).
\end{spex}

\begin{sol}[spsol]
    For any $g \in G$, let $\wt g \in \pcal$ denote the element such that $g \in \wt g$.
	We now show that $\wt 1 \leq G$.
	Since $1 \in \wt 1$, $\wt 1$ is nonempty.
	Suppose $g, h \in \wt 1$.
	Then
    \[
    ...
    To show that the elements of $\pcal$ are the cosets of $\wt 1$, let $g \in G$ with $\bar g \in G/\wt 1$.
